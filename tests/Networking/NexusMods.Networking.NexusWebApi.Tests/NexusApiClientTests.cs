...
public class NexusApiClientTests
{
    ...
    {
        var downloadLinks = await _nexusApiClient.CollectionDownloadLinksAsync(CollectionSlug.From("iszwwe"), RevisionNumber.From(469)); 

        downloadLinks.StatusCode.Should().Be(HttpStatusCode.OK);
    }
}
